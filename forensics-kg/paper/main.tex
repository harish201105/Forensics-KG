...
\item A \emph{validity-aware evaluation methodology} for forensic LLM/KG systems:
the schema-compliance / consistency / validity distinction above, instantiated as
a documented-fact protocol that breaks circular evaluation and a ground-truth-%
alignment analysis that explains \emph{why} some multimodal forensic benchmarks
cannot validate the task they appear to (\cref{sec:setup,sec:results}).
...
\item A conservative, provenance-preserving cross-case entity-resolution method
using embedding-blocked candidate generation and name/role/context verification
with non-destructive \rel{same\_as} links (\cref{sec:resolution}).
...
records each event's calendar date in ISO form; an earlier evaluation that read
only the free-text event \emph{name} under-counted recovery at $\mathrm{F1}=0.09$,
conflating ``stored in a separate field'' with ``not extracted''---exactly the
schema-compliance-versus-validity distinction this paper argues for
...
extracted location match the single gold principal location?---is $0.39$
($11/28$ cases); much of the gap to the precision-driven $\mathrm{F1}$ ($0.27$) is
additional, genuinely-mentioned locations counted as false positives, and many
``misses'' are \emph{more specific} than the gold (\cref{sec:erroranalysis}).
...
binary detection metric (the dataset is all impact-spatter) on a single dataset and
the prompt was not separately held out, so it is a focused positive result rather
than a broad image-forensics claim. Wound \emph{detection} (present vs.\ absent) over-detects
