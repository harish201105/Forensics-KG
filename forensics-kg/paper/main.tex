...
\thanks{\textit{Working draft compiled \today\ (rev.\ v13). Remove this note for camera-ready.}}%
}
...
Over three runs, the system recovers
...
\item A graph-centred reasoning layer---semantic retrieval, grounded
NL-to-graph querying, evidence-chain hypothesis support, and a
...
\label{sec:baselines}
We run two controlled ablations: ontology-guided extraction vs.\ a naive,
same-LLM JSON baseline (\cref{sec:ablation}), which isolates the schema guidance
of \cref{eq:valid,eq:adm}; and a CV-only vs.\ LLM-only vs.\ CV$+$LLM ablation of
the bloodstain image pipeline (\cref{sec:results}). The remaining single-component
comparisons (removing the graph; ungrounded querying, \cref{eq:patterns}; and
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}P{0.6cm}P{2.0cm}P{4.5cm}@{}}
    ...
    RQ4 & \cref{sec:res_eval}, \cref{tab:resolution} & precision-oriented \rel{same\_as}: $27/28$ accepts correct ($1$ common-name merge), recall $0.15$; recovers the $3$ genuine shared suspects \\
    \bottomrule
  \end{tabular}
\end{table}
...
lacks is \emph{principal-location selection}, not place recognition.
...
Candidate-set $\mathrm{F1}$ is thus $0.26$, quantifying the high-precision /
low-recall trade-off the design intends over a $5.4\times$ larger search space
than the ten-case pilot---a candidate-set, not a global, benchmark (should-link
labels follow the objective rule of \cref{tab:resolution}). Links are
...
\paragraph*{Recall is a tunable design choice: a calibrated tiered resolver}
The deployed resolver fixes \emph{one} deliberately conservative operating point---a
name match must be corroborated by context or role. Sweeping the decision over the
same labelled candidate pool shows candidate-set recall is tunable, not a fixed
limitation (\cref{tab:frontier}). Of the $176$ true cross-case identities in the
pool, $162$ are \emph{category} nodes (e.g.\ crime types, where a name match carries
no collision risk) and $14$ are \emph{instance} nodes (persons, locations, weapons).
Relaxing the decision to accept on name identity lifts candidate-set recall from
$0.15$ to $1.00$ at precision $0.99$; the \emph{single} residual error is the
common-name person collision noted above (``Ajay Kumar''). A \emph{type-aware
tiered} resolver---auto-merge identical category nodes (safe), and route identical
\emph{instance} names that lack corroboration to a human-review queue
($\approx\!0.1$ pairs/case)---isolates that collision, giving precision
$\approx\!1.0$ at recall $1.0$; accepting \emph{weak} name matches (name-score
$\ge0.5$) instead degrades precision to $0.77$. Two honest caveats: this is
\emph{candidate-set} recall (absolute recall stays bounded by the embedding-blocking
floor, a separate and also-tunable stage), and the pool is dominated by category
merges, so the tier that matters for genuine cross-case \emph{instance} links
(shared suspects, recurring agencies) is the type-aware one. The contribution is the
method and its \emph{calibrated frontier}: a practitioner picks recall against a
quantified review budget rather than accepting a single fixed point.

\begin{table}[t]
  \centering
  \caption{Tiered cross-case resolver: the precision / recall / review-load
  frontier over the candidate pool (recall is candidate-set, bounded by blocking).
  The deployed system uses the conservative point; the type-aware tier recovers
  near-complete in-pool recall at precision $\approx\!1.0$ by routing $\sim\!0.1$
  pairs/case to review.}
  \label{tab:frontier}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{@{}P{4.45cm}ccc@{}}
    \toprule
    \textbf{Operating point} & \textbf{P} & \textbf{R} & \textbf{Rev./case} \\
    \midrule
    Conservative (corroboration req.; \emph{deployed}) & 0.96 & 0.15 & --- \\
    Relaxed (accept on name identity)        & 0.99 & 1.00 & --- \\
    Type-aware tiered (auto-merge categories; review instances) & $\approx$1.00 & 1.00 & 0.1 \\
    Aggressive (name-score $\ge0.5$)         & 0.77 & 1.00 & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
usable weapon across any case (it never tags, e.g., \emph{cyanide}, the Koodathayi
murder agent, as a \node{Weapon}/method), whereas the ontology-guided extractor
recovers weapons at recall $0.66$---lifts \node{CrimeType} from $0.75$ to $0.91$
...
evaluation that scored \node{TimeEvent} from the event name alone---see
...
Remaining comparisons---removing the graph, ungrounding the query generator, and
disabling resolution---are left to future work (\cref{sec:future}).
...
\paragraph*{Why the location score is low} \node{Location} is low for a
structural reason: the gold records the \emph{principal} location while the
extractor returns every place mentioned, so genuinely-relevant secondary places
count as false positives. The fix is structural---bind the case to its principal
location (via \rel{occurred\_at}) and score against that---a concrete, near-term
improvement rather than a fundamental limitation. It shows that part of the value
gap lies in evaluation design, not only in extraction---the same lesson the
\node{TimeEvent} \texttt{date}-field correction (\cref{sec:erroranalysis}) makes
plain.
...
immediate priority: completing the controlled component matrix beyond the two
ablations reported here (removing the graph, ungrounding the query generator, and
disabling resolution); and expanding real-case validation beyond 28 cases with
formal significance testing. \emph{Expert
annotation}: extend the two-annotator gold (\cref{sec:iaa}) with
\emph{forensic-expert} annotators to enable rigorous, attribute-level validation. \emph{More real data}: incorporate larger
